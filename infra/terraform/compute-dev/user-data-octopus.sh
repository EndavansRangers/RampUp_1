#!/bin/bash
# User Data for Octopus Deploy Server
...
echo "=================================================="
echo "Octopus Deploy Server - User Data Script"
echo "=================================================="
...
aws ssm put-parameter \
    --name "/${project}/${env}/octopus/instance-id" \
    --value "$INSTANCE_ID" \
    --type "String" \
    --overwrite \
    --region "${region}" || true
...
aws ssm put-parameter \
    --name "/${project}/${env}/octopus/private-ip" \
    --value "$PRIVATE_IP" \
    --type "String" \
    --overwrite \
    --region "${region}" || true
...
echo "Octopus Deploy will be configured by Ansible playbook"
echo "Access will be available at: http://$PRIVATE_IP:8080"
echo ""
echo "=================================================="
...
# Signal completion
aws ssm put-parameter \
    --name "/${project}/${env}/octopus/user-data-status" \
    --value "completed" \
    --type "String" \
    --overwrite \
    --region "${region}" || true
